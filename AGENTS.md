...
- On the Mac: unlock uses Touch ID or the Mac login password, right-click offers the swipe actions, entry forms open page-sized, and Cmd-[ goes back. A pushed page's navigation bar content moves into the window title bar.
...
- Status: `/api/v1/documents` goes live when this work lands on `main`, because the production gateway deploys from `main`. The Release Mac app is installed at `/Applications/SAVY.app` on the Mac Studio; Adam signs in once. The phone's own records reach the server after this build is installed on the iPhone, which needs phone availability coordinated first.
