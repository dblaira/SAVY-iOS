...
const fixtureCandidates = [
  join(__dirname, "../../../understood-app/fixtures/ontology/adam-beliefs-triples.json"),
  join(__dirname, "../../../understood-app/ontology/adam-beliefs-triples.json"),
  join(__dirname, "../fixtures/adam-beliefs-triples.json"),
];
...
function resolveFixturePath(): string {
  ...
  throw new Error(
    "adam-beliefs-triples.json not found. Run `npm run export:adam-beliefs` in understood-app first."
  );
}
...
async function connectPool(): Promise<InstanceType<typeof pg.Pool>> {
  ...
  if (!host || host.startsWith("postgresql://")) {
    throw new Error("Set DATABASE_URL or AURORA_HOST to import adam-beliefs triples into Aurora");
  }
...
async function main() {
  ...
  try {
    ...
    try {
      ...
      const { rows: connectionRows } = await client.query<{ count: string }>(
        `SELECT COUNT(DISTINCT subject)::text AS count
         FROM savy.rdf_triples
         WHERE graph_iri = 'https://understood.app/graph/personal'
           AND predicate = 'http://www.w3.org/1999/02/22-rdf-syntax-ns#type'
           AND object = 'https://understood.app/ontology#Connection'
           AND source_app IN ('understood', 'recall')`
      );
      const connections = Number.parseInt(connectionRows[0]?.count ?? "0", 10);

      console.log(
        `Imported ${inserted} new triple rows from ${fixturePath}. Authoritative Connection beliefs in graph: ${connections}.`
      );
    } finally {
      client.release();
    }
